#!/usr/bin/env python
# This is python module to merge FastANN files, and randomize them.
# Then it stores input and output on one line for easy fetching  
# S.Chekanov (ANL)
...
if (len(sys.argv)<3):
      ...
      print "merge_data_flattener.py [ouput file], input1, input2, input3.."
      sys.exit()
        ...
        with open(fname) as infile:
            for line in infile:
                if (nn>0): 
                        total=total+1 
                        np=line.split();
                        if (len(np)==mSize): line=line.replace("\n","# ") 
                        outfile.write(line)
                nn=nn+1

total=total/2

print "Shufalling tmp file.."
import os
cmd="cat /tmp/tmp.txt | shuf > /tmp/a; mv /tmp/a  /tmp/tmp.txt"
os.system(cmd)
